// Credit Card Authorization (Non-Prorated) Document Handler
...
/**
 * Generate Credit Card Authorization (Non-Prorated) document
 */
export async function handleGenerateCreditCardAuthNonProrated(
  ...
  try {
    // Load template
    const templateInfo = getTemplateByAction('generate_credit_card_auth_nonprorated');
    if (!templateInfo) {
    ...
    // Calculate payment totals (same as prorated)
    ...
    // Prepare template data (same as prorated)
    const templateData = {
    ...
    // Generate filename
    const filename = generateFilename('recurring_credit_card_auth-nonprorated', payload.agreementNumber);
