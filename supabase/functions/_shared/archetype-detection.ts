...
export async function getUserArchetypeSignals(
  ...
  const completedTransactions = transactions.filter(t => t.status === 'accepted');
  const requestedTransactions = transactions.filter(t => t.requester_id === userId);
  const receivedTransactions = transactions.filter(t => t.receiver_id === userId);

  const avgTransactionValue = completedTransactions.length > 0
    ? completedTransactions.reduce((sum, t) => sum + (t.proposed_price || 0), 0) / completedTransactions.length
    : 0;

  // Calculate willingness to pay (ratio of proposed to base price)
  const willingnessToPay = requestedTransactions.length > 0
    ? requestedTransactions.reduce((sum, t) => {
        const ratio = t.base_price > 0 ? (t.proposed_price || 0) / t.base_price : 1;
        return sum + Math.min(ratio, 2);  // Cap at 2x
      }, 0) / requestedTransactions.length / 2  // Normalize to 0-1
    : 0.5;

  // Calculate price rejection rate
  const priceRejections = requestedTransactions.filter(t =>
  ...
  const responseTimes = receivedTransactions
    .filter(t => t.responded_at && t.created_at)
    .map(t => {
  ...
  const acceptanceRate = receivedTransactions.length > 0
    ? receivedTransactions.filter(t => t.status === 'accepted').length / receivedTransactions.length
    : 0.5;

  // Calculate request frequency (last 90 days)
  const ninetyDaysAgo = new Date();
  ninetyDaysAgo.setDate(ninetyDaysAgo.getDate() - 90);
  const recentRequests = requestedTransactions.filter(t =>
  ...
  const accommodationHistory = receivedTransactions.filter(t => t.status === 'accepted').length;

  // Calculate reciprocity ratio
  const reciprocityRatio = requestedTransactions.length > 0
    ? receivedTransactions.filter(t => t.status === 'accepted').length /
      requestedTransactions.filter(t => t.status === 'accepted').length
    : 1;
...
export function detectUserArchetype(signals: ArchetypeSignals): UserArchetype {
  ...
  const confidence = Math.min(0.95, Math.max(0.4, margin + 0.5));
