...
/**
 * Calculate urgency multiplier using interpolation from simulation data
 * Uses empirically-validated multipliers from revenue optimization simulation
 *
 * @param daysOut - Days until check-in (0-90)
 * @param steepness - Urgency curve steepness (recommended: 2.0, currently unused but kept for API compatibility)
...
export function calculateUrgencyMultiplier(
  ...
  // Use interpolation from simulation-validated lookup table
  const multiplier = interpolateUrgencyMultiplier(clampedDays);

  // Apply max cap (10x) and min floor (1.0x)
  const MAX_MULTIPLIER = 10.0;
  return Math.min(MAX_MULTIPLIER, Math.max(1.0, multiplier));
}

/**
 * Interpolate urgency multiplier from lookup table
 * Uses linear interpolation between data points from revenue simulation
 *
 * @param daysOut - Days until check-in
 * @returns Interpolated multiplier
 */
function interpolateUrgencyMultiplier(daysOut: number): number {
  // Lookup table keys (sorted descending: 90, 60, 30, ...)
  const lookupKeys = Object.keys(URGENCY_MULTIPLIER_EXAMPLES)
    .map(Number)
    .sort((a, b) => b - a);

  // Exact match - return directly
  if (URGENCY_MULTIPLIER_EXAMPLES[daysOut as keyof typeof URGENCY_MULTIPLIER_EXAMPLES] !== undefined) {
    return URGENCY_MULTIPLIER_EXAMPLES[daysOut as keyof typeof URGENCY_MULTIPLIER_EXAMPLES];
  }

  // Find bracketing values for linear interpolation
  for (let i = 0; i < lookupKeys.length - 1; i++) {
    const upperDays = lookupKeys[i];     // e.g., 30 days
    const lowerDays = lookupKeys[i + 1]; // e.g., 21 days

    if (daysOut <= upperDays && daysOut >= lowerDays) {
      const upperMultiplier = URGENCY_MULTIPLIER_EXAMPLES[upperDays as keyof typeof URGENCY_MULTIPLIER_EXAMPLES];
      const lowerMultiplier = URGENCY_MULTIPLIER_EXAMPLES[lowerDays as keyof typeof URGENCY_MULTIPLIER_EXAMPLES];

      // Linear interpolation: y = y0 + (y1 - y0) * (x - x0) / (x1 - x0)
      const ratio = (daysOut - lowerDays) / (upperDays - lowerDays);
      return lowerMultiplier + (upperMultiplier - lowerMultiplier) * ratio;
    }
  }

  // Beyond range: use boundary values
  if (daysOut > lookupKeys[0]) {
    // Beyond farthest milestone (90+ days) = base price
    return URGENCY_MULTIPLIER_EXAMPLES[lookupKeys[0] as keyof typeof URGENCY_MULTIPLIER_EXAMPLES];
  }

  // Below closest milestone (< 1 day) = peak price
  return URGENCY_MULTIPLIER_EXAMPLES[lookupKeys[lookupKeys.length - 1] as keyof typeof URGENCY_MULTIPLIER_EXAMPLES];
}
...
export function checkPriceAlerts(
  ...
  // Significant increase alert (if multiplier increased by 30%+ from previous check)
  if (previousMultiplier && multiplier >= previousMultiplier * 1.3) {
    alerts.push({
      type: 'doubling',
