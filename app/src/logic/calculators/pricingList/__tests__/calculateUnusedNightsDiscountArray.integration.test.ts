...
describe('calculateUnusedNightsDiscountArray', () => {
  ...
  describe('Default Parameters', () => {
    it('should use default UNUSED_NIGHTS_DISCOUNT_MULTIPLIER', () => {
      ...
      expect(result[0]).toBe(0); // 1 night = no volume discount
      expect(result[6]).toBe(6 * expectedMultiplier); // 7 nights = maximum discount
    });

    it('should use default 0.03 discount when not specified', () => {
      const result = calculateUnusedNightsDiscountArray({});
      expect(result).toEqual([0, 0.03, 0.06, 0.09, 0.12, 0.15, 0.18]);
    });
  });
  ...
  describe('Linear Discount Calculation', () => {
    it('should calculate discounts linearly with default multiplier', () => {
      const result = calculateUnusedNightsDiscountArray({});
      // At 1 night: (1-1) * 0.03 = 0 (no volume discount)
      expect(result[0]).toBe(0);
      // At 2 nights: (2-1) * 0.03 = 0.03
      expect(result[1]).toBe(0.03);
      // At 3 nights: (3-1) * 0.03 = 0.06
      expect(result[2]).toBe(0.06);
      // At 4 nights: (4-1) * 0.03 = 0.09
      expect(result[3]).toBe(0.09);
      // At 5 nights: (5-1) * 0.03 = 0.12
      expect(result[4]).toBe(0.12);
      // At 6 nights: (6-1) * 0.03 = 0.15
      expect(result[5]).toBe(0.15);
      // At 7 nights: (7-1) * 0.03 = 0.18 (maximum discount)
      expect(result[6]).toBe(0.18);
    });

    it('should calculate discounts with custom baseDiscount', () => {
      const result = calculateUnusedNightsDiscountArray({ baseDiscount: 0.05 });
      expect(result[0]).toBe(0); // (1-1) * 0.05
      expect(result[1]).toBe(0.05); // (2-1) * 0.05
      expect(result[2]).toBe(0.10); // (3-1) * 0.05
      expect(result[3]).toBe(0.15); // (4-1) * 0.05
      expect(result[4]).toBe(0.20); // (5-1) * 0.05
      expect(result[5]).toBe(0.25); // (6-1) * 0.05
      expect(result[6]).toBe(0.30); // (7-1) * 0.05
    });
  ...
  describe('Rounding Precision', () => {
    it('should round to 4 decimal places', () => {
      ...
      expect(result[0]).toBe(0); // (1-1) * 0.0333 = 0
      expect(result[5]).toBe(0.1665); // (6-1) * 0.0333 = 0.1665
      expect(result[6]).toBe(0.1998); // (7-1) * 0.0333 = 0.1998
    });

    it('should handle repeating decimals correctly', () => {
      const result = calculateUnusedNightsDiscountArray({ baseDiscount: 0.01 });
      expect(result[0]).toBe(0); // (1-1) * 0.01 = 0
      expect(result[3]).toBe(0.03); // (4-1) * 0.01 = 0.03
      expect(result[6]).toBe(0.06); // (7-1) * 0.01 = 0.06
    });
  });
  ...
  describe('Discount Progression', () => {
    it('should increase discount as nights increase', () => {
    ...
    it('should have zero discount at 1 night', () => {
      const result = calculateUnusedNightsDiscountArray({ baseDiscount: 0.03 });
      expect(result[0]).toBe(0);
    });

    it('should have maximum discount at 7 nights', () => {
      const result = calculateUnusedNightsDiscountArray({ baseDiscount: 0.03 });
      const maxDiscount = Math.max(...result);
      expect(result[6]).toBe(maxDiscount);
    });
  });
  ...
  describe('Real-World Scenarios', () => {
    it('should calculate standard length-of-stay discounts', () => {
      const result = calculateUnusedNightsDiscountArray({
    ...
    it('should incentivize booking more nights', () => {
      ...
      // 7 nights should get MORE discount than 1 night (volume benefit)
      expect(sevenNightDiscount).toBeGreaterThan(oneNightDiscount);
      expect(oneNightDiscount).toBe(0);
    });
  });
});
