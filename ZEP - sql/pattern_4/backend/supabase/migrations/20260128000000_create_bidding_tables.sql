...
-- Tables: bidding_sessions, bids, bidding_participants, bidding_results
...
-- Example session (for testing only)
...
COMMENT ON TABLE bidding_sessions IS 'Competitive bidding sessions between Big Spender pairs';
COMMENT ON TABLE bidding_participants IS 'Users participating in bidding sessions';
COMMENT ON TABLE bids IS 'Individual bids placed during sessions';
COMMENT ON TABLE bidding_results IS 'Final outcomes and financial settlement';
...
COMMENT ON COLUMN bidding_sessions.round_duration_seconds IS 'Duration of each bidding round in seconds';
COMMENT ON COLUMN bidding_participants.max_auto_bid_amount IS 'eBay-style proxy bidding maximum';
COMMENT ON COLUMN bids.is_auto_bid IS 'TRUE if bid was placed automatically by proxy system';
COMMENT ON COLUMN bidding_results.loser_compensation_amount IS '25% of winning bid paid to loser';
