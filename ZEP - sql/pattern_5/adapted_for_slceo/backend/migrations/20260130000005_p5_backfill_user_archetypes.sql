-- ============================================================================
-- PATTERN 5: FEE TRANSPARENCY - USER ARCHETYPE BACKFILL (ADAPTED)
-- ============================================================================
-- Migration: Backfill archetype data based on historical behavior
-- Adapted for SLCEO: Uses Bubble-style column naming
-- Date: 2026-01-30
-- ============================================================================
...
BEGIN
    ...
    IF v_avg_response_time_hours IS NOT NULL THEN
        ...
        ELSIF v_avg_response_time_hours > 168 THEN
...
BEGIN
    ...
    LOOP
        BEGIN
            SELECT * INTO v_scores FROM calculate_user_archetype_scores(v_user.id);
    ...
    RAISE NOTICE 'Backfill complete: % users updated, % skipped', v_updated_count, v_skipped_count;
END $$;
...
BEGIN
    SELECT COUNT(*) INTO v_total_users FROM public.user;
