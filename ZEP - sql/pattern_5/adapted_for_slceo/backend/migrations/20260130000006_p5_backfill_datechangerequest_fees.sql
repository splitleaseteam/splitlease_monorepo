-- ============================================================================
-- PATTERN 5: FEE TRANSPARENCY - DATE CHANGE REQUEST FEE BACKFILL (ADAPTED)
-- ============================================================================
-- Migration: Backfill fee breakdown for existing date change requests
-- Adapted for SLCEO: Uses bookings_leases table with "Total Rent" column
-- Date: 2026-01-30
-- ============================================================================
...
-- ADAPTED: Uses bookings_leases table with "Total Rent" column (Bubble-style)
DO $$
DECLARE
    v_request RECORD;
...
BEGIN
    ...
    LOOP
        BEGIN
            -- ADAPTED: Get "Total Rent" from bookings_leases (Bubble-style naming)
            ...
            IF v_monthly_rent IS NULL OR v_monthly_rent <= 0 THEN
                RAISE NOTICE 'Skipping request % - invalid Total Rent (lease: %)',
...
BEGIN
    ...
    RAISE NOTICE '----------------------------------------';
    RAISE NOTICE 'Average base price (Total Rent): $%', ROUND(COALESCE(v_avg_base_price, 0), 2);
