...
class HealthChecker:
    ...
    def run_health_checks(self, tests: Dict[str, Dict[str, Any]], service_order: List[str]) -> str:
        ...
        # Format message(Converted from UTC to Eastern)
        # now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        eastern = pytz.timezone('America/New_York')
        now = datetime.now(pytz.UTC).astimezone(eastern).strftime("%Y-%m-%d %H:%M:%S")
        status_emoji = ':white_check_mark:' if unhealthy == 0 else ':x: ERROR'

        message = [
            f"{status_emoji}: :split-lease-purple-circle-1: SplitLease API Health Report - {now}",
            "",
