# Recurring Credit Card Authorization Form Prorated

from flask import Blueprint, request, jsonify, send_file
from .generator import CreditCardAuthGenerator
import logging
import os
import json
import traceback

credit_card_auth = Blueprint('credit_card_auth_prorata', __name__)
logger = logging.getLogger(__name__)

# Initialize generator with base directory
base_dir = os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
generator = CreditCardAuthGenerator(base_dir)

@credit_card_auth.route('/recurring_card_auth_prorated', methods=['POST'])
def generate_authorization():
    """
    Generate a recurring credit card authorization form (prorated) and upload to Google Drive
    ...
    try:
        # Log request details
        logger.info("=== New Recurring Credit Card Auth Request ===")
        ...
        for field in currency_fields:
            try:
                ...
                return jsonify({
                    'success': False,
                    'error': f'Invalid currency value for {field}: {data[field]}'
                }), 400
...
@credit_card_auth.route('/template', methods=['GET'])
def get_template():
