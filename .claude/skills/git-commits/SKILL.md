...
description: |
  ...
  [hostname][username][type] commit message
...
```
[<hostname>][<username>][<type>] <commit_message>
```
...
|-----------|-------------|---------|
| `hostname` | Machine identifier | `SPLIT-LEASE-6` |
| `username` | Git user identity | `sl6-sharath` |
...
```bash
# Feature commit
git commit -m "[SPLIT-LEASE-6][sl6-sharath][feat] add proposal submission workflow"

# Bug fix
git commit -m "[SPLIT-LEASE-6][sl6-sharath][fix] resolve FK constraint error in listing updates"

# Documentation
git commit -m "[SPLIT-LEASE-6][sl6-sharath][docs] update CLAUDE.md with git-commits skill"

# Chore
git commit -m "[SPLIT-LEASE-6][sl6-sharath][chore] update dependencies to latest versions"
```
...
```bash
git commit -m "[SPLIT-LEASE-6][sl6-sharath][feat] implement booking confirmation flow
...
2. Get username: `git config user.name`
3. Determine commit type based on changes
4. Write clear, imperative commit message
5. Use heredoc format for multi-line commits

## Heredoc Format (Recommended)

```bash
git commit -m "$(cat <<'EOF'
[SPLIT-LEASE-6][sl6-sharath][feat] implement user authentication
