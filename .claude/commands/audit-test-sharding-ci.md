...
|----------|------|--------|
| P0 | Add matrix strategy | Parallel execution |
| P0 | Set fail-fast: false | See all failures |
| P1 | Configure blob reporter | Enable merging |
| P1 | Add merge reports job | Unified report |
| P2 | Share auth state | Avoid duplicate logins |
| P2 | Add browser caching | Faster job setup |
| P3 | Dynamic shard count | Scale with tests |
| P3 | Conditional sharding | Optimize PR builds |

## File Organization
```
.github/
└── workflows/
    ├── e2e.yml           # E2E with sharding
    ├── unit.yml          # Unit tests with sharding
    └── full-suite.yml    # Complete CI pipeline
e2e/
...
- name: Report shard status
  ...
  run: echo "::error::Shard ${{ matrix.shard }} failed"
```

### Reproduce Failed Shard Locally
```bash
# Run same shard that failed
